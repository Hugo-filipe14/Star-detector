...
pub(crate) fn construct_closed_polygon(contour: &Contour<u32>) -> LineString<f32> {
    // Craete a new line string that connects all points in the contour. This can create either an open or a close shape
    let mut line_string = LineString::from_iter(contour.points.iter().map(|point| Coord {
        x: point.x as f32,
        y: point.y as f32,
    }));
    // If it is an open shape, close the shape to create a polygon. This does nothing otherwise
    line_string.close();
    line_string
}
...
pub(crate) fn filter_map_contour_to_star_centers(contour: &Contour<u32>) -> Option<StarCenter> {
    // If there are no points in the contour it is not a star
    if contour.points.is_empty() {
        return None;
    }
    if contour.points.len() == 1 {
        // If there is only 1 point in the contour consider it to be the center of the star of size 1px
    ...
    // Otherwise, construct a polygon around the star based on contour information
    let polygon = construct_closed_polygon(contour);
    // Find the center of gravity if this polygon (centroid)
    let center = polygon.centroid().unwrap();
    // Find the radius of the star based on maximun distance between the centroid and any of the points in contour
    let radius = polygon.points().fold(0., |distance, point| {
        point.euclidean_distance(&center).max(distance)
    });
    // If the radius is kess than 1px or more than 24px we reject it as a non-star
...
pub(crate) fn find_star_centres_and_size(image: &GrayImage) -> Vec<StarCenter> {
    ...
    contours
        .iter()
        // Iterate over all contours and create a list of star center ans size data
