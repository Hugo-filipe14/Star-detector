...
impl ThresholdingExtensions for StarDetect {
    fn optimize_threshold_for_star_count(&self, min_star_count: usize) -> u8 {
        ...
        while star_count < min_star_count {
            ...
            // This means that there are fewer stars the we hoped for
            if threshold == 0 {
                panic!("Could not find enough stars in the image");
            }
            // Reduce threshold to 95% of its previous value
            // Using this, we check finer and finer differences in threshold for each iteration
            threshold = (0.95 * threshold as f32) as u8;
            // Clone the source data since we need to modify it without affecting original data
            let mut source = self.clone();
            // Binarize the image using the current threshold
            source.binarize(threshold);
    ...
    fn binarize(&mut self, threshold: u8) {
        ...
        for pixel in self.source.iter_mut() {
            if *pixel > threshold {
                // If pixel intensity is greater than threshold set it to maximum intensity instead
                *pixel = u8::MAX;
            } else {
